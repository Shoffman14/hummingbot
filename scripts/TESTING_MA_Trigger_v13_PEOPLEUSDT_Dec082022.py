...
class MATriggers(ScriptStrategyBase):
    ...
    order_amount_usd = Decimal(2000)    # each order should be ~$2,000
    base = "PEOPLE"
    quote = "USDT"
    ...
    def on_tick(self):
        p = self.connectors["bybit_perpetual"].get_price("PEOPLE-USDT", True)
        ...
        if (fast_ma > slow_ma) & (self.pingpong == 0):
            ...
            self.logger().info(f"{self.amount}-PEOPLE bought") #[12/7/22]: calculated amount should populate
            self.pingpong = 1
        ...
        #: logic for bearish cross
            ...
            self.logger().info(f"{self.amount}-PEOPLE sold")  # [12/7/22]: calculated amount should populate
            self.pingpong = 0
        else:
