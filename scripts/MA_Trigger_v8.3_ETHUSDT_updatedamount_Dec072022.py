...
from hummingbot.client.hummingbot_application import HummingbotApplication  # added 12/8/22 after testing errors
...
class MATriggers(ScriptStrategyBase):
    ...
    def on_tick(self):
        ...
        if (fast_ma > slow_ma) & (self.pingpong == 0):
            ...
            self.logger().info(f"{self.amount}-ETH bought") #[12/7/22]: calculated amount should populate
            self.pingpong = 1
        ...
        #: logic for bearish cross
            ...
            self.logger().info(f"{self.amount}-ETH sold")  # [12/7/22]: calculated amount should populate
            self.pingpong = 0
        else:
