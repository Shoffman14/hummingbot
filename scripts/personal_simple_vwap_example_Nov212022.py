...
from hummingbot.core.event.events import OrderFilledEvent, OrderType, TradeType, PositionAction
from hummingbot.core.rate_oracle.rate_oracle import RateOracle
from hummingbot.strategy.script_strategy_base import ScriptStrategyBase


class PersonalVWAPExample(ScriptStrategyBase):
    last_ordered_ts = 0
    vwap: Dict = {"connector_name": "bybit_perpetual", "trading_pair": "ETH-USDT", "is_buy": True,
                  "total_volume_usd": 100000, "price_spread": 0.001, "volume_perc": 0.001, "order_delay": 10}
        ...
        if self.last_ordered_ts < (self.current_timestamp - self.vwap["order_delay_time"]):
            if self.vwap.get("status") is None:
                ...
                self.place_order(
                    ...
                    position_action=PositionAction.OPEN) # I added this myself b/c I will be running on perpetual_pair
    ...
    def init_vwap_stats(self):
        ...
        vwap["real_quote_volume"] = vwap["target_base_volume"]
    ...
    def create_order(self) -> OrderCandidate:
        ...
        # Query the cumulative volume until the price is affected by spread
        ...
        vwap_order = OrderCandidate(
            ...
            amount=amount,
            price=price,
            position_action=PositionAction.OPEN)    # I added 'position_action' b/c my code is for perpetuals pair
        return vwap_order
    ...
    def place_order(self,
                    ...
                    price=Decimal("NaN"),
                    position_action=PositionAction  # I added this line; not sure if values are correct
                    ):
        if is_buy:
            self.buy(connector_name, trading_pair, amount, order_type, price, position_action) # I added 'position_action'
        else:
            self.sell(connector_name, trading_pair, amount, order_type, price, position_action) # I added 'position_action'

    def did_fill_order(self, event: OrderFilledEvent):
        # Listens to fill order event to log it and notify the Hummingbot application.
        # If you set up Telegram bot, you will get a notification there as well.
        if event.trading_pair == self.vwap["trading_pair"] and event.trade_type == self.vwap["trade_type"]:
            ...
            if math.isclose(self.vwap["delta"], 1, rel_to1=1e-5):
    ...
    def format_status(self) -> str:
        # Returns status of the current strategy on user balance and current active orders. This function is called
        ...
        lines.extend(["", " Balances:"] + [" " + line for line in df.to_string(index=False).split("\n")])

        try:
            df = self.active_orders(df)
