...
class MATriggers(ScriptStrategyBase):
    ...
    def on_tick(self):
        p = self.connectors["bybit_perpetual"].get_price("SOL-USDT", True)
        ...
        if (fast_ma > slow_ma) & (self.pingpong == 0):
            ...
            self.logger().info(f"{self.amount}-SOL bought") #[12/7/22]: calculated amount should populate
            self.pingpong = 1
        ...
        #: logic for bearish cross
            ...
            self.logger().info(f"{self.amount}-SOL sold")  # [12/7/22]: calculated amount should populate
            self.pingpong = 0
        else:
