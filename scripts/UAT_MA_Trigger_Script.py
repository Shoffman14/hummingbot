...
class MATriggersv2(ScriptStrategyBase):
    ...
    def on_tick(self):
        p = self.connectors["bybit_perpetual"].get_price(f"{base}-{quote}")
        entry_price = self.connectors["bybit_perpetual"].get_entry_price(f"{base}-{quote}")
        ...
        if (fast_ma > slow_ma) & (self.pingpong == 0):
            conversion_rate = RateOracle.get_instance().get_pair_rate(f"{sef.base}-USD")
